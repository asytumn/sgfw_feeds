...
for target_address in "${TARGET_ADDRESSES[@]}"; do
    ...
    if [ "${CLIENT_TARGET}" != "" ] ; then
        ...
        ## Setup testshell
        ##
        ssh_copy_id=$(ssh root@$target_address "which ssh-copy-id || which /root/ssh-copy-id")
        if [ "$ssh_copy_id" = "" ] ; then
            # Copy script locally to target
            scp $(which ssh-copy-id) root@$target_address:/root
        fi
